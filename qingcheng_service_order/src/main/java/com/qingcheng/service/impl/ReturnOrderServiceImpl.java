...
public class ReturnOrderServiceImpl implements ReturnOrderService {
    ...
    public void agreeRefund(String id, Integer money, Integer adminId) {
        ...
        returnOrder.setAdminId(adminId);//设置管理员
        returnOrder.setDisposeTime(new Date());//设置处理时间
        returnOrderMapper.updateByPrimaryKeySelective(returnOrder);//更新数据库

        //调用支付接口平台退款
    }

    public void rejectRefound(String id, String remark, Integer adminId) {
        ReturnOrder returnOrder = returnOrderMapper.selectByPrimaryKey(id);
        if (returnOrder==null){
            throw new RuntimeException("退款订单不存在!");
        }
        ...
        //修改属性
        returnOrder.setRemark(remark);//驳回原因
